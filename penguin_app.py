...
if island == 'Biscoe':
    ...
    island = 2
classifier = st.sidebar.selectbox("Classifier", ('RandomForestClassifer' , 'SVC' , 'Logistic Regression'))
if st.sidebar.button("Predict"):
    if classifier == 'SVC':
        species_type = prediction(svc_model , island , b_len , b_depth , f_len , b_mass , sex)
        score = svc_score
    elif classifier == 'RandomForestClassifier':
        species_type = prediction(rf_clf , island , b_len , b_depth , f_len , b_mass , sex)
        score = rf_clf_score
    else:
        species_type = prediction(log_reg , island , b_len , b_depth , f_len , b_mass , sex)
        score = log_reg_score
